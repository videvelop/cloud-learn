...
echo EC2 instance profile is $INSTANCE_PROFILE
...
Create() 
{
    ...
    echo  "ssh-keygen -f ~/.ssh/${EC2_KEY_PAIR}"
    ssh-keygen -f ~/.ssh/${EC2_KEY_PAIR}
    ...
    --public-key-material fileb://~/.ssh/${EC2_KEY_PAIR}.pub"
...
Delete() 
{
    ...
    echo "===="
    echo "Delete keypair"
    echo "  aws ec2 delete-key-pair --key-name ${EC2_KEY_PAIR}"

    aws ec2 delete-key-pair --key-name ${EC2_KEY_PAIR} 
    if [ $? != 0 ] ; then echo "Some error..." >&2 ; GetYN ; fi
